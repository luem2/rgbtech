...
function validation(input) {
  ...
  if (!validationPasswords.password.test(input.nuevaContraseña)) {
    errors.nuevaContraseña = "Debe contener almenos 8 caracteres, debe tener una mayuscula, una miniscula y un numero"
  }
...
export default function RecoverPassword() {
  ...
  <div onSubmit={handlerSubmit} className="mb-6">
    <label for="email" className="text-gray-700 font-medium">New Password</label>
    ...
    <input  value={input.nuevaContraseña} onChange={(e) => handlerChange(e)} name="nuevaContraseña"  type={showpassword} placeholder="Enter your password" className="block rounded-md border border-gray-300 py-2 px-4 my-2 shadow-sm w-full"/>
	
	{errors.nuevaContraseña && <p className="text-red-600 "><small>{errors.nuevaContraseña}</small></p>}
	
  </div>
  <div className="mb-6">
    <label for="password" className="text-gray-700 font-medium">Confirm password</label>
    <input value={input.confirmacionDeContraseña} onChange={(e) => handlerChange(e)} name="confirmacionDeContraseña" type={showpassword} placeholder="Enter your password" className="block rounded-md border border-gray-300 py-2 px-4 my-2 shadow-sm w-full"/>
	{ errors.confirmacionDeContraseña && <p className="text-red-600"><small>{errors.confirmacionDeContraseña}</small></p>}
	{ errors.concidencia && <p className="text-red-600"><small>{errors.concidencia}</small></p>}
  </div>
  <div className="flex justify-between items-center mb-3">
    <div className="mt-auto">
    </div>
  </div>
  <button disabled={errors.confirmacionDeContraseña || errors.confirmacionDeContraseña || errors.concidencia ? true : false} type="submit" onClick={(e) => handlerSubmit(e)} className="bg-red-400 hover:bg-red-500 py-2 rounded text-white my-2 px-4 w-full">Continue</button>
  </form>
  {/* <br />
